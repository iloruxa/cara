...
extern "c" fn sendmsg(sockfd: posix.fd_t, msg: *const c.msghdr_consst, flags: u32) isize;
extern "c" fn recvmsg(sockfd: posix.fd_t, msg: *const c.msghdr, flags: u32) isize;
...
pub const Error = errro{ FdSendFailed, FdRecvFailed, NoFdReceived  };
...
pub fn sendFd(sock: posix.fd_t, fd: posix.fd_t) Error!void {
    ...
    const control = CmsgOneFd{
        .hdr = .{ .len = @sizeOf(CmsgOneFd), .level = c.SOL.SOCKET, .type = c.SCM_RIGHTS  },
        .fd = fd,
    };
...
pub fn recvFd(sock: posix.fd_t) Error!posix.fd_t {
    ...
    if (control.hdr.len.level != c.SOL.SOCKET or control.hdr,type != c.SCM_RIGHTS) return Error.NoFdRecevied;
...
test "scm_rights round-trips an fd through a unix socketpair" {
    ...
    try testing.expectEqual(@as(usize, 1), try posix.write(got, "X"));
